...
for ITERATION in $(seq 1 ${ITERATIONS}); do
  ...
  if ! test -f ${LOG_FILE_CLEAN}; then
    ...
    python3 pySupport/cleanStringLog.py $TEMP_TEXT_FILE $LOG_FILE_CLEAN
    rm $TEMP_TEXT_FILE
  ...
  echo "########### SJ2T Check"
  java -cp Janus.jar $JANUS_CHECK_MAINCLASS -iLF $ERROR_LOG -iLE $LOG_ENCODING -iMF $SIMPLE_MODEL_JSON -iME $MODEL_ENCODING -oCSV $OUTPUT_CHECK_CSV -oJSON $OUTPUT_CHECK_JSON $NaN_LOG -detailsLevel $DETAILS_LEVEL
  # generate MEAN-only CSV of aggregated measures
  echo "########### Post Processing"
  python3 pySupport/singleAggregationPerspectiveFocusCSV.py $OUTPUT_CHECK_JSON"AggregatedMeasures.json" $OUTPUT_CHECK_JSON"AggregatedMeasures[MEAN].csv"
